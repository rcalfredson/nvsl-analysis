...
# Only the home-vector analysis (all, top 20%, and bottom 50% by default)
RUN_TURNBACK_HOME_VECTOR_ALIGNMENT_ONLY=1 scripts/run_analysis_matrix.sh

# Compare Control top 20% versus bottom 20% across all radius pairs
RUN_TURNBACK_HOME_VECTOR_ALIGNMENT_ONLY=1 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_TOP_SLI_FRACTION=0.2 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_BOTTOM_SLI_FRACTION=0.2 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_PLOT_CONTROL_TOP_BOTTOM=1 \
  scripts/run_analysis_matrix.sh

# Compare top 20% versus bottom 20% within every cohort and radius pair.
# Cohort brackets span compact 3/5, 8/10, and 13/15 mm x-axis labels.
RUN_TURNBACK_HOME_VECTOR_ALIGNMENT_ONLY=1 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_TOP_SLI_FRACTION=0.2 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_BOTTOM_SLI_FRACTION=0.2 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_PLOT_ALL_GROUPS_TOP_BOTTOM=1 \
  scripts/run_analysis_matrix.sh

# Show a title on that optional plot (hidden by default)
RUN_TURNBACK_HOME_VECTOR_ALIGNMENT_ONLY=1 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_PLOT_CONTROL_TOP_BOTTOM=1 \
  TURNBACK_HOME_VECTOR_ALIGNMENT_CONTROL_TOP_BOTTOM_SHOW_TITLE=1 \
  scripts/run_analysis_matrix.sh
...
regeneration. The optional
`TURNBACK_HOME_VECTOR_ALIGNMENT_TOP_SLI_FRACTION` and
`TURNBACK_HOME_VECTOR_ALIGNMENT_BOTTOM_SLI_FRACTION` set the learner subset
sizes; their defaults are `0.2` and `0.5`. Each value must be greater than zero
and at most one, and their sum must not exceed one. Bundle names, plot names,
and labels incorporate the configured percentages, so differently sized
subsets do not overwrite one another. The
`TURNBACK_HOME_VECTOR_ALIGNMENT_PLOT_CONTROL_TOP_BOTTOM=1` output compares the
configured Control top and bottom SLI subsets within each 3/5, 8/10, and 13/15
mm radius pair, using the subset bundles generated by the same run. Its title
is hidden by default; set
`TURNBACK_HOME_VECTOR_ALIGNMENT_CONTROL_TOP_BOTTOM_SHOW_TITLE=1` to show it. Use
`TURNBACK_HOME_VECTOR_ALIGNMENT_PLOT_ALL_GROUPS_TOP_BOTTOM=1` for a direct
top-versus-bottom comparison in every configured cohort. That plot groups the
three radius pairs beneath a bracket labeled with the cohort name, leaving only
the compact radius pair on each x-axis tick. Set
`TURNBACK_HOME_VECTOR_ALIGNMENT_ALL_GROUPS_TOP_BOTTOM_SHOW_TITLE=1` to show its
optional figure title. Use
`DUAL_CIRCLE_TURNBACK_IMG_FORMAT=pdf` or
