...
from src.analysis.sync_bucket_presence_filters import (
    ...
    exp_pi_threshold_filter_result,
...
def _default_mask(*, excluded=False):
    ...
    mask[DEFAULT_EXP_PI_FILTER_TRAINING - 1][0][
        DEFAULT_EXP_PI_FILTER_SYNC_BUCKET - 1
    ] = bool(excluded)
    return mask
...
def test_exp_pi_threshold_eligibility_mask_and_payload_report_per_video_results():
    ...
    eligible = exp_pi_threshold_eligibility_mask(vas, opts)
    payload = exp_pi_threshold_filter_payload(
...
def test_combined_eligibility_mask_requires_enabled_pi_threshold_filter():
    opts = SimpleNamespace(require_exp_pi_threshold_bucket=True, piTh=10)
    vas = [
        _va_with_counts([0, 0, 0, 0, 6], [0, 0, 0, 0, 4]),
        _va_with_counts([0, 0, 0, 0, 5], [0, 0, 0, 0, 4]),
...
def test_mask_by_exp_pi_threshold_filter_masks_first_dimension_rows():
    ...
    masked = mask_by_exp_pi_threshold_filter(values, [True, False])

    np.testing.assert_allclose(masked[0], [[1.0, 2.0]])
    assert np.isnan(masked[1]).all()


def test_mask_by_exp_pi_threshold_filter_rejects_length_mismatch():
    with pytest.raises(ValueError, match="eligibility mask length"):
        mask_by_exp_pi_threshold_filter(np.ones((2, 3)), [True])
