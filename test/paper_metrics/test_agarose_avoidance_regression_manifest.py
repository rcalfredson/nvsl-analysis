...
MANIFEST = Path("test/reference/bundles/agarose_avoidance_paper_manifest.json")
...
def test_paper_agarose_avoidance_bundle_manifest_matches_available_exports():
    manifest = load_manifest(MANIFEST)
    missing_paths = _missing_bundle_paths(manifest)
    if missing_paths:
        pytest.skip(
            "Paper agarose avoidance export bundles are not available locally: "
            + ", ".join(missing_paths)
        )
    ...
    if missing_keys:
        ...
        pytest.skip(
            "Paper agarose avoidance export bundles predate required metadata: "
            + detail
        )
