...
JPG_X, PNG_X, AVI_X, AD_AVI_X, TXT_X = (
    re.compile(p, re.IGNORECASE)
    for p in (r"\.jpg$", r"\.png$", r"\.avi$", r"AD\.avi$", r"\.txt$")
)
...
def python2_round(n, ndigits=None):
    """
    Rounds using Python 2's away-from-zero behavior.

    n: Number to round.
    ndigits: Decimal places (default is nearest integer).
    ...
    if ndigits is None:
        ...
        factor = 10.0**ndigits
        return round(n * factor + 0.5 - (n < 0)) / factor


# returns rounded ints (as tuple if there is more than one) for the given
#  values; can be passed iterable with values
def intR(*val):
    if val and isinstance(val[0], collections.abc.Iterable):
        val = val[0]
    t = tuple(int(python2_round(v)) for v in val)
...
def get_valid_filename(name):
    """
    adapted from Django web framework
    ...
    s = re.sub(r"(?u)[^-\w.]", "", s)
    if s in {"", ".", ".."}:
        raise "Could not derive file name from '%s'" % s
    return s
...
def join(withStr, l, lim=None, p=None, end=False):
    ...
    ff = "" if p is None else "%%.%if" % p
    return withStr.join(
        ff % e if ff and isinstance(e, (float, np.floating)) else str(e) for e in l
    )
...
def _toTuple(t1, t2):
    if isinstance(t2, (int, np.int64, float, str)):
        t2 = len(t1) * (t2,)
    return t2
...
# concatenates the given arrays (not yet used)
def concatA(*args):
...
# @jit(nopython=True)
def angleDiff(theta1, theta2, absVal=False):
    result = normAngles(theta1 - theta2, useRadians=False)
    if absVal == True:
...
def normAngles(a, useRadians=True):
    if useRadians:
        normed = np.mod(a + np.pi, 2 * np.pi) - np.pi
    else:
        normed = np.mod(a + 180, 360) - 180
    return normed


def rotate_pts(
    pts, cos_of_angles, sin_of_angles, origins=None, multiple_pts_per_rotation=False
):
    """rotate a set of points by one or more angles.

    input:
      - pts: matrix of points (one point per row, columns are X and Y)
             type: NumPy ndarray
      - cos_of_angles: vector of cosines of rotation angles
      - sin_of_angles: vector of sines of rotation angles
      - origins: matrix of origins about which to perform each rotation. Number
                 of rows must match number of angles; columns are X and Y.
                 Defaults to rotation about the origin (0, 0).
      - multiple_pts_per_rotation: if set to false, each point is rotated by each
                                   rotation angle (one-to-one mapping where the
                                   number of angles must match the number of
                                   points). If set to true, each point is rotated
                                   by all the rotation angles.

    output:
     - array of rotated points:
          - shape if multiple_pts_per_rotation is true: (# angles, # points, 2)
          - shape if multiple_pts_per_rotation is false: (# angles, 2)
    """
    if origins is None:
        origins = np.array([[0, 0] for _ in range(len(cos_of_angles))])
    if multiple_pts_per_rotation:
        pts = np.reshape(
            np.repeat(pts, origins.shape[0], 0), (pts.shape[0], origins.shape[0], 2)
        )
    pts = pts - origins
    pts_rotated = np.array(pts)
    pts_rotated[..., 0] = pts[..., 0] * cos_of_angles - pts[..., 1] * sin_of_angles
    pts_rotated[..., 1] = pts[..., 1] * cos_of_angles + pts[..., 0] * sin_of_angles
    pts_rotated = pts_rotated + origins
    if multiple_pts_per_rotation:
        pts_rotated = np.transpose(pts_rotated, (1, 0, 2))
    return pts_rotated


def hyst(x, th_lo, th_hi, initial=False):
    """find threshold-crossing events with hysteresis: events begin when the value
    of vector x is <= threshold th_lo and continue until the value is >= threshold th_hi.

    input:
      - x: vector of values to compare with the thresholds
      - th_lo: threshold at which to begin a crossing event.
      - th_hi: threshold at which to end a crossing event.
      - initial: value of the output vector before the first event begins.

    output:
       - array of bool values indicating whether event is ongoing.
    source: https://stackoverflow.com/a/23291658
    """
    hi = x >= th_hi
    lo_or_hi = (x <= th_lo) | hi
...
def closest_pts_to_lines(line_xs, line_ys, pts):
    """finds point on a line segment that's closest to another point,
    and the distance between them, with vectorization to support multiple sets
    of segments and points.

    input:
      - line_xs: ndarray of X values of the line segments.
                 shape: [# segments, 2]
      - line_ys: ndarray of Y values of the line segments.
                 shape: [# segments, 2]
      - pts: points to connect to the line segments (one point per segment)
             shape: [# segments, 2]
             note: for this variable, columns represent X and Y, unlike for
             line_xs and line_ys.

    output:
     - 2-tuple:
          - distances between segments and points
          - points on the segments closest to the given points
    """
    u = (
        (pts[:, 0] - line_xs[:, 0]) * (line_ys[:, 0] - line_xs[:, 0])
        + (pts[:, 1] - line_xs[:, 1]) * (line_ys[:, 1] - line_xs[:, 1])
    ) / (
        np.power(line_ys[:, 0] - line_xs[:, 0], 2)
        + np.power(line_ys[:, 1] - line_xs[:, 1], 2)
    )
    closest_pts_on_seg = np.vstack(
        (
            line_xs[:, 0] + u * (line_ys[:, 0] - line_xs[:, 0]),
            line_xs[:, 1] + u * (line_ys[:, 1] - line_xs[:, 1]),
        )
    ).T
    return np.linalg.norm(closest_pts_on_seg - pts, axis=1), closest_pts_on_seg
...
def rdp(xy, epsilon, useRdpPkg=False):
    ...
    if useRdpPkg:
        ...
        if len(xy) == 1:
            ...
            sxy = np.array(
                sg.LineString(xy).simplify(epsilon, preserve_topology=False).coords
            )
            si = getIdxs(sxy)
            if _TEST_RDP:
...
def pickle(obj, fn, make_backup=False, verbose=""):
    if make_backup:
        backup(fn, "B" in verbose)
    f = open(fn, "wb")
    pkl.dump(obj, f, -1)
    f.close()
...
def unix_to_windows(unix_path):
    json_file_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "drive_mapping.json"
    )
    ...
    if os.path.exists(json_file_path):
        ...
        for key, value in mapping.items():
            if unix_path.startswith(key):
                return unix_path.replace(key, value).replace("/", "\\")
...
def writeCommand(lf, csvStyle=False, inclRev=True):
    # Quote arguments if they contain spaces or special characters
...
def xy2Pts(*xy):
    if len(xy) == 2 and isinstance(xy[0], collections.abc.Iterable):
        pts = np.array(xy).T
    else:
...
# returns gray image using the given channel or if channel is None, original
#  image; if channel is 3, same as toGray()
def toChannel(img, channel):
    return (
        img
        if channel is None
        else (toGray(img) if channel == 3 else img[:, :, channel])
    )
...
def combineImgs(
    ...
    for i, img in enumerate(imgs):
        ...
        xL, yL = int(d / 2) + c * (wA + d), int(
            d / 2
        ) + hdrLH + hdrLD + hdrH + hd + r * (h + d + hdrH + hd)
        if img is not None:
            h1, w1 = img.shape[:2]
...
def heatmap(
    mp, xform=np.log, xySeps=None, mapCheck=False, thres=None, colormap=cv2.COLORMAP_JET
):
    xySeps = xySeps or ([], [])
    h, w = mp.shape[:2]
    imgL = getImg(h, w)
    ...
    for x2 in xySeps[0] + [w]:
        ...
        for y2 in ys:
            ...
            imgL[y1:y2, x1:x2] = cv2.applyColorMap(img, colormap)
            y1 = y2
        x1 = x2
    return imgL
