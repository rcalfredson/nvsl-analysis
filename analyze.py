...
from src.plotting.between_reward_tortuosity_wall_scatter import (
    BetweenRewardTortuosityWallScatterConfig,
    BetweenRewardTortuosityWallScatterExporter,
)
...
from src.utils.local_config import (
    load_local_analyze_config,
    parse_local_bool,
)
...
g.add_argument(
    "--plot-rewards-xlabel",
    default=None,
    help="Optional x-axis label override for plots generated by plotRewards().",
)
g.add_argument(
    "--plot-rewards-ylabel",
    default=None,
    help="Optional y-axis label override for plots generated by plotRewards().",
)
g.add_argument(
    "--sli-ylim-mode",
    ...
    default=None,
    help=(
        "Y-axis policy for time-dependent SLI plots. Overrides SLI_YLIM_MODE "
        "from .analyze.local.env. Supplying either SLI limit flag implies fixed."
    ),
)
g.add_argument(
    "--sli-ylim-min",
    ...
    metavar="MIN",
    help=(
        "Fixed lower y-axis limit for time-dependent SLI plots. Overrides "
        "SLI_YLIM_MIN from .analyze.local.env."
    ),
)
g.add_argument(
    "--sli-ylim-max",
...
g.add_argument(
    "--btw-rwd-dist-sli-group",
    type=str,
    choices=("top", "bottom"),
    default=None,
    help=(
        "Restrict between-reward distance histograms/export to a selected "
...
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-export",
    type=str,
    default=None,
    help=(
        "Export segment-level wall-contact percentage and tortuosity values as "
        "a compressed .npz for scatter/hexbin plotting."
    ),
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-training",
    type=int,
    default=1,
    help="Training to export for tortuosity-vs-wall-contact scatter data (1-based).",
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-scope",
    type=str,
    choices=("full", "return_leg"),
    default="full",
    help=(
        "Segment window for tortuosity-vs-wall-contact export. 'full' uses the "
        "entire between-reward segment; 'return_leg' starts at the max-distance frame."
    ),
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-mode",
    type=str,
    choices=(
        "path_over_max_radius",
        "path_over_displacement",
    ...
    default="path_over_max_radius",
    help="Tortuosity definition used for wall-contact scatter export.",
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-exclude-nonwalking-frames",
    action="store_true",
    help=(
        "Exclude non-walking frames before computing segment tortuosity and "
        "wall-contact percentage."
    ),
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-min-walk-frames",
    type=int,
    default=2,
    help="Minimum walking frames required per segment (default: %(default)s).",
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-exclude-reward-endpoints",
    action="store_true",
    help="Use frames strictly between consecutive rewards for each segment metric.",
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-min-radius-mm",
    type=float,
    default=0.0,
    help="Minimum max radial distance required for path_over_max_radius tortuosity.",
)
g.add_argument(
    "--btw-rwd-tortuosity-wall-scatter-min-displacement-mm",
    type=float,
    default=0.0,
    help="Minimum net displacement required for chord-based tortuosity modes.",
)
...
g.add_argument(
    ...
    "--y-tick-spacing",
    dest="sync_bucket_y_tick_spacing",
    type=positive_finite_float,
    default=None,
    metavar="F",
    help=(
        "Override the major y-tick spacing for per-sync-bucket time-series "
        "plots generated by plotRewards and the default bundle pipeline. "
...
g.add_argument(
    "--event-chain-reward-circle-color",
    choices=("grey", "black"),
    default="grey",
    help=(
        "Reward-circle outline color for plots generated by EventChainPlotter. "
        "Default: %(default)s."
    ),
)
g.add_argument(
...
g.add_argument(
    ...
    action="store_true",
    help=(
        "deprecated compatibility option; single-group reward-index, SLI, and "
        "rewards-per-distance AUC labels are now shown automatically."
    ),
)
g.add_argument(
    "--audit-rpid-auc",
...
g.add_argument(
    "--export-turnback-outer-radius-sli-bundle",
    type=str,
    default=None,
    help=(
...
g.add_argument(
    ...
    default=None,
    help=(
        "Optional export-specific sync-bucket skip for turnback-outer-radius "
...
g.add_argument(
    ...
    help=(
        ...
        "last K sync buckets per training for turnback-outer-radius bundles. "
...
g.add_argument(
    ...
    action="store_true",
    help=(
        "For return-prob-excursion-bin exports, discard reward-exit episodes "
        "whose frame span overlaps wall contact."
    ),
)
g.add_argument(
...
def headerForType(va, tp, calc):
    ...
    if "dbr" in tp:
        ...
        return "\ndual-circle reward turnback ratio by sync bucket:"
    elif tp == "rpd":
    ...
    for f in plot_fs:
        ...
        for i, t in enumerate(trns):
            ...
            for g in range(ng):  # group
                ...
                # For a single plotted group, compare paired experimental/yoked
                # AUCs when both curves are present.  Experimental-only and
                # already-differenced plots instead test their signed AUCs
                # against zero.  Run after the final curve for training index 1.
                show_single_group_auc = (
                    ng == 1
                    and g == ng - 1
                    and f == plot_fs[-1]
                    and i == 1
                ...
                if show_single_group_auc:
                    primary_fly = 0 if 0 in plot_fs else plot_fs[0]
                    primary_series = getVals(0, None, False, f1=primary_fly)
                    reference_series = (
                        getVals(0, None, False, f1=1)
                        if not diff_tp and 0 in plot_fs and 1 in plot_fs
                        else None
                    )
                    auc_result = compute_single_group_auc_test(
                        primary_series,
                    ...
                    if auc_result is not None:
                        if useMidPlotAUCVerticalAlignment:
                            ...
                            base_y_for_auc = (
                                -0.79 if nosym else pch(-0.55, -0.46)
                            )
                            avoid_ys = [
                                (
                                    txt._final_y_
                                    if hasattr(txt, "_final_y_")
                                    else getattr(txt, "_y_", None)
                                )
                                for text_list in lbls.values()
                                for txt in text_list
                                if hasattr(txt, "_y_")
                                or hasattr(txt, "_final_y_")
                            ]
            ...
            if f == 0 or not joinF:
                ...
                if not P or i == 0:
                    ylabels = dict(
                        ...
                        rpid="SLI",
                        rpipd="SLI",
                        rpd="rewards per distance $(\\mathrm{m}^{-1})$",
                    ...
                    ylabel_text = (
                        str(opts.plot_rewards_ylabel)
                        if opts.plot_rewards_ylabel is not None
                        else ylabels.get(y_label_tp, "reward index")
                    )
                    plt.ylabel(
                        maybe_sentence_case(
                            pcap(ylabel_text)
    ...
    for metric in (
        ...
        try:
            ...
            plot_metric_sli_bundle_data(
                ...
                y_tick_spacing=getattr(
                    opts, "sync_bucket_y_tick_spacing", None
                ),
            )
...
def postAnalyze(vas):
    ...
    for tc in tcs:
        ...
        if tp == "rpid" and sli_ser is not None:
            ...
            if getattr(opts, "memory_saver", False):
                ...
                sli_t1_sb2_sb5_mean_ser = compute_sli_per_fly(
                    raw_4,
                    training_idx=0,
                ...
                plot_cross_fly_correlations(
                    ...
                    sli_t1_sb2_sb5_mean_values=sli_t1_sb2_sb5_mean_ser,
    ...
    if va.circle and getattr(opts, "btw_rwd_dist_hist", False):
        ...
        sli_group = getattr(opts, "btw_rwd_dist_sli_group", None)

        # Backward compatibility for older top-only flag, if present
        if sli_group is None and getattr(opts, "btw_rwd_dist_top_sli", False):
            sli_group = "top"
        ...
        if sli_group == "top":
            if saved_top is None:
                print(
                    "[btw_rwd_dists] WARNING: top-SLI restriction requested "
                    "but no top-SLI group is available; falling back to all flies."
                )
            else:
                vas_for_hist = [vas[i] for i in saved_top]
            ...
            if saved_bottom is None:
                print(
                    "[btw_rwd_dists] WARNING: bottom-SLI restriction requested "
                    "but no bottom-SLI group is available; falling back to all flies."
                )
            else:
                vas_for_hist = [vas[i] for i in saved_bottom]
                print(
                    "[btw_rwd_dists] restricting histograms to "
                    f"{len(vas_for_hist)} bottom-SLI flies"
                )
    ...
    out_wall_scatter = getattr(opts, "btw_rwd_tortuosity_wall_scatter_export", None)
    if va.circle and out_wall_scatter:
        training_index = int(
            getattr(opts, "btw_rwd_tortuosity_wall_scatter_training", 1) or 1
        ) - 1
        if training_index < 0:
            training_index = 0
        cfg = BetweenRewardTortuosityWallScatterConfig(
            export_npz=str(out_wall_scatter),
            ...
            metric_mode=str(
                getattr(
                    opts,
                    "btw_rwd_tortuosity_wall_scatter_mode",
                    "path_over_max_radius",
                )
                or "path_over_max_radius"
            ),
            segment_scope=str(
                getattr(opts, "btw_rwd_tortuosity_wall_scatter_scope", "full")
                or "full"
            ),
            exclude_nonwalking_frames=bool(
                getattr(
                    opts,
                    "btw_rwd_tortuosity_wall_scatter_exclude_nonwalking_frames",
                    False,
                )
            ),
            exclude_reward_endpoints=bool(
                getattr(
                    opts,
                    "btw_rwd_tortuosity_wall_scatter_exclude_reward_endpoints",
                    False,
                )
            ),
            min_walk_frames=int(
                getattr(opts, "btw_rwd_tortuosity_wall_scatter_min_walk_frames", 2)
                or 2
            ),
            min_displacement_mm=float(
                getattr(
                    opts,
                    "btw_rwd_tortuosity_wall_scatter_min_displacement_mm",
                    0.0,
                )
                or 0.0
            ),
            min_radius_mm=float(
                getattr(opts, "btw_rwd_tortuosity_wall_scatter_min_radius_mm", 0.0)
                or 0.0
            ),
        )
        exporter = BetweenRewardTortuosityWallScatterExporter(
            vas=vas, opts=opts, gls=gls, cfg=cfg
        )
        exporter.export_npz(str(out_wall_scatter))
...
if __name__ == "__main__":
    ...
    if getattr(opts, "return_prob_excursion_bin_exclude_wall_contact", False):
        if getattr(opts, "wall", None) is None:
            print(
    ...
    if getattr(opts, "turnback_excursion_bin_exclude_wall_contact", False):
        if getattr(opts, "wall", None) is None:
            print(
                f"[turnback-excursion-bin] enabling --wall={WALL_CONTACT_DEFAULT_THRESH_STR} "
                "because --turnback-excursion-bin-exclude-wall-contact was set"
            )
            opts.wall = WALL_CONTACT_DEFAULT_THRESH_STR
    ...
    if getattr(opts, "btw_rwd_tortuosity_wall_scatter_export", None):
        if getattr(opts, "wall", None) is None:
            print(
                f"[btw_rwd_tortuosity_wall_scatter] enabling --wall={WALL_CONTACT_DEFAULT_THRESH_STR} "
                "because --btw-rwd-tortuosity-wall-scatter-export was set"
            )
            opts.wall = WALL_CONTACT_DEFAULT_THRESH_STR
