...
The focused 20-column matched-cohort rewards-per-distance (experimental minus
yoked) export is generated by
`scripts/run_rpd_exp_minus_yok_chamber_graphpad.sh`; the guide documents its
video-list mapping, selective refresh controls, and metric-window options.
...
overlays—SLI set-operation groups, and turn/home-vector SLI selections use this
policy. Scalar SLI values stored in metric+SLI bundles also use the shared
policy and record it as bundle metadata. Cross-fly correlations may override it
with `--corr-sli-min-valid-sync-buckets`. A bucket-level SLI is valid only when
...
| Averaged trajectory heatmaps | `--pltHm`; optional `--hm-periods pre,training,post --num-trainings 2 --hm-sync-bucket 5` | `imgs/heatmaps.png`, `imgs/heatmaps2.png` | Defaults to training and post rows. Pre-training is opt-in; maps are normalized per video and averaged across valid flies/videos by fly role. |
...
For example, to plot matched final-10-minute pre-training, T2 SB5, and
post-training occupancy heatmaps for a cohort:

```bash
python analyze.py -v '/path/to/cohort/*.avi' -f 0-1 \
  --pltHm --hm-periods pre,training,post --hm-pre-minutes 10 \
  --num-trainings 2 --hm-sync-bucket 5 \
...
| Agarose dual-circle avoidance + SLI | `--export-agarose-sli-bundle exports/group_a_agarose_sli.npz` | `scripts/plot_com_sli_bundles.py`; stats via `scripts/stats_agarose_sli_bundles.py` or `scripts/stats_agarose_sli_anova.py` | `--metric agarose`; add `--agarose-sli-include-pre` when pre windows matter. Add `--agarose-virtual-control` to store the paired, inter-well spatial control in the same bundle. |
...
| Wall contacts per reward interval totals | `--wall-contacts-per-reward-interval-total-bars --wall-contacts-per-reward-interval-total-export exports/group_a_wall_interval_totals.npz` | `scripts/plot_overlay_training_metric_scalar_bars.py` | Scalar bar overlays with optional stats. Also supports `--wall-contacts-per-reward-interval-total-metric wall_contact_trajectory_length` for wall-contact-only path length. |
