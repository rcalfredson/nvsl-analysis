...
PREFIX_DEFAULT = "exp_pi_threshold_filter"
...
def summarize_bundle(path: Path, *, prefix: str = PREFIX_DEFAULT) -> tuple[dict, list[dict]]:
    with np.load(path, allow_pickle=True) as data:
        ...
        target_exp_counts = _array(
            data, f"{prefix}_target_exp_count", np.full(eligible.size, np.nan)
        ).reshape(-1)
        target_ctrl_counts = _array(
            data, f"{prefix}_target_ctrl_count", np.full(eligible.size, np.nan)
        ).reshape(-1)
        target_count_sums = _array(
            data, f"{prefix}_target_count_sum", np.full(eligible.size, np.nan)
        ).reshape(-1)
        if target_exp_counts.size != eligible.size:
            target_exp_counts = np.full(eligible.size, np.nan)
        if target_ctrl_counts.size != eligible.size:
            target_ctrl_counts = np.full(eligible.size, np.nan)
        if target_count_sums.size != eligible.size:
            target_count_sums = np.full(eligible.size, np.nan)
        ...
        threshold_failed = excluded_reasons.get("pi_threshold_failed", 0)
        other_excluded = int((~eligible).sum()) - missing_target_bucket - threshold_failed
        ...
        for idx, (
            ...
            elif reason == "pi_threshold_failed":
                category = "pi_threshold_failed"
            else:
            ...
            rows.append(
                {
                    ...
                    "pi_threshold": summary["pi_threshold"],
                    ...
                    "turnback_episode_counts": (
                        ...
                        else ";".join(map(str, episode_overlap["counts"][idx].reshape(-1)))
                    ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Summarize the experimental final-PI threshold filter accounting stored "
    ...
    parser.add_argument(
        ...
        help=f"Filter payload prefix in the bundle (default: {PREFIX_DEFAULT}).",
    )
    parser.add_argument(
        "--excluded-csv",
        default=None,
    ...
    for summary in summaries:
        ...
        print(
            ...
            f" pi_threshold={summary['pi_threshold']},"
        ...
        print(
            ...
            f" pi_threshold_failed={summary['n_pi_threshold_failed']},"
    ...
    if len(summaries) > 1:
        ...
        total_threshold = sum(s["n_pi_threshold_failed"] for s in summaries)
        ...
        print(
            ...
            f" pi_threshold_failed={total_threshold},"
            f" other={total_other}"
        )
