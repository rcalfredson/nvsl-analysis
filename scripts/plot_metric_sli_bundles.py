...
def main():
    ...
    p.add_argument(
        "--metric-condition",
        "--turnback-mode",
        dest="metric_condition",
        default="exp",
        choices=["exp", "ctrl", "exp_minus_ctrl"],
        help=(
            "For metrics with exp/ctrl variants (e.g. turnback, agarose, "
            "reward_lgturn_*, reward_lv): which condition to plot. "
            "--turnback-mode is a deprecated alias."
        ),
    )
    p.add_argument(
        "--delta-vs",
    ...
    p.add_argument(
        "--hide-auc", action="store_true", help="Hide per-training AUC test labels."
    )
    p.add_argument(
        "--include-pre",
        action="store_true",
