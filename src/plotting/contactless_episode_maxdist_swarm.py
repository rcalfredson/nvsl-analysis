...
from src.plotting.palettes import ACCENT_TEAL, ACCENT_WINE, NEUTRAL_DARK
...
STATUS_ORDER = (False, True)
STATUS_LABELS = {
    False: "No wall contact",
    True: "Wall contact",
}
STATUS_COLORS = {
    False: ACCENT_TEAL,
...
def _as_bool(series: pd.Series) -> pd.Series:
    ...
    if not bool(valid.all()):
        bad = sorted(set(series.loc[~valid].astype(str)))
...
def load_episode_csvs(paths: list[str], *, include_excluded: bool = False):
    ...
    for path in paths:
        ...
        frame["contains_wall_contact"] = _as_bool(
            frame["contains_wall_contact"]
        )
        frame["included_in_metric"] = _as_bool(frame["included_in_metric"])
        frame["max_distance_from_reward_center_mm"] = pd.to_numeric(
            frame["max_distance_from_reward_center_mm"],
            errors="coerce",
        )
    ...
    data = data.loc[
        np.isfinite(data["max_distance_from_reward_center_mm"])
    ].copy()
    return data, group_order


def plot_contactless_episode_maxdist_swarm(
    data: pd.DataFrame,
    *,
    group_order: list[str],
    title: str | None = None,
    ylabel: str = "Maximum distance from\nreward center (mm)",
...
def save_figure(fig, out: str, *, image_format: str) -> str:
    ...
    print(f"[contactless_episode_maxdist_swarm] wrote {path}")
    return str(path)
