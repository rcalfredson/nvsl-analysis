...
_LEGACY_KEY_ALIASES = {
    "running_pi_sli_curve": "cum_reward_sli_curve",
    "running_pi_sli_ticks": "cum_reward_sli_ticks",
    "running_pi_sli_pi_threshold": "cum_reward_sli_pi_threshold",
    "running_pi_sli_reward_pi_exp": "cum_reward_sli_reward_pi_exp",
    "running_pi_sli_reward_pi_yoked": "cum_reward_sli_reward_pi_yoked",
    "running_pi_sli_total_actual_rewards": "cum_reward_sli_total_actual_rewards",
    "running_pi_sli_skip_first_sync_buckets": "cum_reward_sli_skip_first_sync_buckets",
    "running_pi_sli_keep_first_sync_buckets": "cum_reward_sli_keep_first_sync_buckets",
}


def _bundle_value(bundle: dict, key: str):
    if key in bundle:
        return bundle[key]
    legacy_key = _LEGACY_KEY_ALIASES.get(key)
    if legacy_key is not None and legacy_key in bundle:
        return bundle[legacy_key]
    raise KeyError(key)


def _bundle_has_key(bundle: dict, key: str) -> bool:
    return key in bundle or _LEGACY_KEY_ALIASES.get(key) in bundle
...
def _infer_total_rewards(bundle):
    if _bundle_has_key(bundle, "running_pi_sli_total_actual_rewards"):
        totals = _bundle_value(bundle, "running_pi_sli_total_actual_rewards")
        return np.asarray(totals, dtype=float).reshape(-1)

    curve = np.asarray(_bundle_value(bundle, "running_pi_sli_curve"), dtype=float)
    ticks = np.asarray(_bundle_value(bundle, "running_pi_sli_ticks"), dtype=float).reshape(-1)
...
def _shared_tick_positions(bundles, max_supported_tick):
    ...
    tick_maps = []
    for b in bundles:
        ticks = np.asarray(_bundle_value(b, "running_pi_sli_ticks"), dtype=float).reshape(-1)
...
def _resolve_min_fly_pct(bundles, min_fly_pct):
    ...
    vals = []
    for b in bundles:
        v = b.get("running_pi_sli_min_fly_pct")
        if v is None:
            vals.append(95.0)
        else:
...
def _metric_spec(metric: str | None) -> tuple[str, str, str]:
    ...
    specs = {
        "sli": (
            "running_pi_sli_curve",
            "SLI",
            "Running SLI by reward count",
        ),
        "reward_pi": (
            "running_pi_sli_reward_pi_exp",
            "Reward PI (experimental fly)",
            "Running reward PI by reward count",
        ),
        "reward_pi_exp": (
            "running_pi_sli_reward_pi_exp",
            "Reward PI (experimental fly)",
        ...
        "reward_pi_yoked": (
            "running_pi_sli_reward_pi_yoked",
            "Reward PI (yoked fly)",
            "Running reward PI by reward count",
        ),
    }
...
def plot_running_pi_sli_bundles(
    bundle_paths,
    out_fn,
    *,
    labels=None,
    ...
    for b in bundles:
        for key in (metric_key, "running_pi_sli_ticks"):
            if not _bundle_has_key(b, key):
                raise ValueError(
    ...
    if ticks0.size == 0:
        raise ValueError(
            "No shared reward-count ticks remain after applying the min-fly support cutoff."
        )
    for gi, b in enumerate(bundles):
        ...
        for which, sub_idx, linestyle in subset_specs:
            ...
            vals_full = np.asarray(_bundle_value(b, metric_key), dtype=float)[sub_idx, :]
    ...
    plt.xlabel("Actual reward count")
    plt.ylabel(y_label)
    plt.title(
...
def plot_cum_reward_sli_bundles(*args, **kwargs):
    return plot_running_pi_sli_bundles(*args, **kwargs)
