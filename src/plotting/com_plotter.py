...
def plot_com_distance(
    ...
    # ----- styling / mapping shared across figs -----
    exp_color = "#1f4da1"
    ctrl_color = "#a00000"
    diff_color = "#1f4da1"
    ...
    # deterministic group → linestyle mapping used across all figures
    style_idx_for_group = {}
    for g_pos, g in enumerate(groups):
        style_idx_for_group[g if multi_group else 0] = g_pos % len(group_linestyles)

    # ===================== helper to render the main plot =====================
    def _make_com_fig(show_ctrl: bool, title_suffix: str, fname_suffix: str):
        ...
        for i, ax in enumerate(axes):
            ...
            for g in groups:
                ...
                # update top envelope using what's actually drawn in this fig
                ...
                # track per-axes y-extents
                ymins.append(np.nanmin(m_exp_i - ci_exp_i))
                ymaxs.append(np.nanmax(m_exp_i + ci_exp_i))
                if show_ctrl and has_ctrl:
                    ymins[-1] = min(ymins[-1], np.nanmin(m_ctrl_i - ci_ctrl_i))
                    ymaxs[-1] = max(ymaxs[-1], np.nanmax(m_ctrl_i + ci_ctrl_i))

            # stars compare groups' EXP only (logic unchanged)
            if multi_group and len(present_in_ax) == 2:
                ...
                a = com4[idx0, i, 0, :]  # EXP for group 0
                b = com4[idx1, i, 0, :]  # EXP for group 1
        ...
        if multi_group:
            ...
            proxy_handles = [
                Line2D(
                    [0],
                    [0],
                    color=(exp_color if not (show_ctrl and has_ctrl) else exp_color),
            ...
            leg = axes[0].legend(
                ...
                labels=proxy_labels,
                loc="upper left",  # temp, will reposition
            ...
            top_star_y = max_star_y_per_ax[0]
            if np.isfinite(top_star_y):
                ...
                if legend_bottom_if_top >= (y_star_axes + pad_axes):
                    ...
                    leg.set_bbox_to_anchor(
                        (0.02, pad_axes), transform=axes[0].transAxes
                    )
            else:
                leg.set_loc("upper left")
                leg.set_bbox_to_anchor(
                    (0.02, 1.0 - pad_axes), transform=axes[0].transAxes
                )
        else:
            for ax in axes:
                ax.legend_.remove() if ax.get_legend() else None

        fig.suptitle(title + (f" — {title_suffix}" if title_suffix else ""))
        plt.tight_layout()

        # save/show
        if outdir:
            os.makedirs(outdir, exist_ok=True)
            base = slugify(title) + (f"__{fname_suffix}" if fname_suffix else "")
            fn = f"{base}.png"
    ...
    # ===================== Figure A: Exp (+Ctrl if present) =====================
    _make_com_fig(
        show_ctrl=has_ctrl,
        title_suffix="",  # keep original title text
        fname_suffix="",  # keep original filename .../<slug>.png
    )

    # ===== EXTRA FIGURE (existing): experimental − yoked (only if controls exist) =====
    if has_ctrl:
        exp_all = com4[:, :, 0, :]
        ctrl_all = com4[:, :, 1, :]
        ...
        for i, ax in enumerate(axes2):
            ...
            for g in groups:
                ...
                ax.plot(
                    xs_i, m_i, marker="o", markersize=ms, linestyle=ls, color=diff_color
                )
                ...
                # stars for EXP-only diffs between groups (same logic as before)
                if multi_group and len(present_in_ax) == 2:
                    g0, g1 = present_in_ax[0], present_in_ax[1]
                    idx0 = np.flatnonzero(gis == g0)
                    idx1 = np.flatnonzero(gis == g1)
        ...
        if multi_group:
            ordered_present = [g for g in sorted(set(gis)) if g in set(gis)]
            proxy_handles = [
                Line2D(
                    [0],
                    [0],
                    color=diff_color,
                    linestyle=group_linestyles[
                        style_idx_for_group[g if multi_group else 0]
                    ],
                    linewidth=2,
                    marker=None,
                )
    ...
    # ===================== Figure C: Exp-only (always) =====================
    _make_com_fig(
        show_ctrl=False,
        title_suffix="experimental only",
        fname_suffix="exp_only",
    )
