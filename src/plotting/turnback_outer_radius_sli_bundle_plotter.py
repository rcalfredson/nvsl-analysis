...
def _mode_values(bundle, mode: str) -> np.ndarray:
    exp = np.asarray(bundle["turnback_outer_radius_ratio_exp"], dtype=float)
    ctrl = np.asarray(bundle["turnback_outer_radius_ratio_ctrl"], dtype=float)
    if mode == "exp":
        return exp
    if mode == "ctrl":
        return ctrl
...
def _bundle_to_exported(
    ...
    *,
    ...
    base_title = "Turnback ratio vs outer-circle radius"
    meta = {
        "y_label": ylabel,
...
def plot_turnback_outer_radius_sli_bundles(
    bundles,
    out,
    *,
    ...
    ylabel = exported[0].meta.get("y_label", "Turnback ratio")
    fig = plot_overlays(
        exported,
        title=title,
