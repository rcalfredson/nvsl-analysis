    ...
    if metric == "ratio":
        ...
        ylabel = (
            ...
            if mode == "exp_minus_ctrl"
                ...
                "Returning excursions per fly (yoked)"
                if mode == "ctrl"
                else "Returning excursions per fly"
            )
        )
        base_title = "Integrated success mass vs radial-delta bin"
    elif metric == "failure":
        ylabel = (
            "Non-returning excursions per fly (exp - yok)"
            if mode == "exp_minus_ctrl"
            else (
                "Non-returning excursions per fly (yoked)"
                if mode == "ctrl"
                else "Non-returning excursions per fly"
            )
        )
        base_title = "Integrated failure mass vs radial-delta bin"
    elif metric == "total":
        ylabel = (
            "Excursions per fly (exp - yok)"
            if mode == "exp_minus_ctrl"
            else (
                "Excursions per fly (yoked)"
                if mode == "ctrl"
                else "Excursions per fly"
            )
        )
        base_title = "Excursion count vs radial-delta bin"
    else:
        raise ValueError(f"Unknown metric={metric!r}")
...
def plot_return_prob_excursion_bin_sli_bundles(
    bundles,
    out,
    *,
