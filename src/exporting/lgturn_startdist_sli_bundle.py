...
def _canonical_bucket_plan_from_syncbucket(vas, opts=None):
    ...
    _dbg(opts, f"[lgturn-startdist-export] canonical plan: n_trn={n_trn}, n_sb={n_sb}, bl_min={bucket_len_min:g}")
...
def _extract_lgturn_startdist_arrays(vas, opts=None):
    ...
    try:
        ...
        _dbg(opts, f"[lgturn-startdist-export] WARNING: failed canonical plan via _syncBucket: {e}")
        n_trn = len(getattr(va0, "trns", []))
        n_sb = 0
        bucket_len_min = np.nan
    ...
    for vi, va in enumerate(vas):
        ...
        if not hasattr(va, "syncMeanLgTurnStartDist") or not hasattr(va, "syncMeanLgTurnStartDistN"):
            try:
                va.bySyncBucketMeanLgTurnStartDist()
            except Exception as e:
                _dbg(opts, f"[lgturn-startdist-export] {vid}: FAILED to compute bySyncBucketMeanLgTurnStartDist ({type(e).__name__}: {e})")
                continue
        ...
        if not isinstance(means, list) or not isinstance(counts, list):
            _dbg(opts, f"[lgturn-startdist-export] {vid}: MISSING or invalid syncMeanLgTurnStartDist/_N")
            continue

        if len(means) != n_trn:
            _dbg(opts, f"[lgturn-startdist-export] {vid}: SKIP (n_trn mismatch: len(means)={len(means)} vs {n_trn})")
            continue
        if len(counts) != n_trn:
            _dbg(opts, f"[lgturn-startdist-export] {vid}: SKIP (n_trn mismatch: len(counts)={len(counts)} vs {n_trn})")
            continue
        ...
        for ti in range(n_trn):
            ...
            if exp_vals.size < n_sb:
                _dbg(opts, f"[lgturn-startdist-export] {vid}: T{ti+1} exp PAD (nb={exp_vals.size} < canonical {n_sb})")
            elif exp_vals.size > n_sb:
                _dbg(opts, f"[lgturn-startdist-export] {vid}: T{ti+1} exp TRUNC (nb={exp_vals.size} > canonical {n_sb})")

            mean_exp[vi, ti, :nb_eff] = exp_vals[:nb_eff]
            if exp_n.size:
                n_exp[vi, ti, :min(n_sb, exp_n.size)] = exp_n[:min(n_sb, exp_n.size)]

        _dbg(opts, f"[lgturn-startdist-export] {vid}: exp finite={np.isfinite(mean_exp[vi]).sum()} / {mean_exp[vi].size}")
        ...
        if len(getattr(va, "trx", [])) > 1:
            for ti in range(n_trn):
                ...
                if ctrl_n.size:
                    n_ctrl[vi, ti, :min(n_sb, ctrl_n.size)] = ctrl_n[:min(n_sb, ctrl_n.size)]

            _dbg(opts, f"[lgturn-startdist-export] {vid}: ctrl finite={np.isfinite(mean_ctrl[vi]).sum()} / {mean_ctrl[vi].size}")

    # Summaries
    _dbg(opts, f"[lgturn-startdist-export] FINAL exp finite={np.isfinite(mean_exp).sum()} / {mean_exp.size}")
    _dbg(opts, f"[lgturn-startdist-export] FINAL ctrl finite={np.isfinite(mean_ctrl).sum()} / {mean_ctrl.size}")

    if n_trn and n_sb:
        per_bucket = np.isfinite(mean_exp).sum(axis=(0, 1))  # (n_sb,)
        per_trn = np.isfinite(mean_exp).sum(axis=(0, 2))  # (n_trn,)
        _dbg(opts, f"[lgturn-startdist-export] exp finite per training: {per_trn.tolist()}")
        _dbg(opts, f"[lgturn-startdist-export] exp finite per bucket (first 10): {per_bucket[:10].tolist()}")
        _dbg(opts, f"[lgturn-startdist-export] exp finite per bucket (last 10): {per_bucket[-10:].tolist()}")
...
def export_lgturn_startdist_sli_bundle(vas, opts, gls, out_fn):
    ...
    _dbg(opts, f"[lgturn-startdist-export] group_label={group_label!r}  n_videos={len(vas_ok)}  noyc={getattr(va0, 'noyc', None)}")

    mean_exp, mean_ctrl, n_exp, n_ctrl, bucket_len_min = _extract_lgturn_startdist_arrays(vas_ok, opts)

    try:
        sli, sli_ts = _compute_sli_scalar_and_timeseries_from_rpid(vas_ok, opts)
    except Exception as e:
        print(f"[export] WARNING: failed to compute SLI for lgturn-startdist bundle: {e}")
    ...
    try:
        video_ids = np.array([getattr(va, "fn", f"va_{i}") for i, va in enumerate(vas_ok)], dtype=object)
    except Exception:
    ...
    np.savez_compressed(
        ...
        sli_use_training_mean=np.array(bool(getattr(opts, "sli_use_training_mean", False))),
    )
    print(f"[export] Wrote lgturn-startdist+SLI bundle: {out_fn} (n={len(vas_ok)})")
    _dbg(opts, "[lgturn-startdist-export] saved keys: lgturn_startdist_exp/ctrl, lgturn_startdistN_exp/ctrl, sli, sli_ts, metadata")
